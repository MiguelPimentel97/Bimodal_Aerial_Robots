...
Joy::Joy() {
  ...
  pnh.param("axis_roll_", axes_.roll, 2);  //Joy 2 3 1 0  Keyboard// 0 1 2 3
  pnh.param("axis_pitch_", axes_.pitch, 3);
  pnh.param("axis_thrust_", axes_.thrust, 1);
  pnh.param("axis_yawrate_", axes_.yawrate, 0); //mm

  pnh.param("axis_direction_roll", axes_.roll_direction, -1);
  pnh.param("axis_direction_pitch", axes_.pitch_direction, 1); //keyboard -1  Joy 1
...
void Joy::JoyCallback(const sensor_msgs::JoyConstPtr& msg) {
  ...
  control_msg_.yaw_rate = msg->axes[axes_.yawrate] * max_.rate_yaw * 5.0 * axes_.yawrate_direction;
